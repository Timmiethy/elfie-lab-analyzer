...
import { getPatientPdf } from '../../services/api';
...
export default function PatientArtifact({
  ...
  const handleExportPdf = async () => {
    try {
      const response = await getPatientPdf(artifact.job_id);
      if (!response.ok) throw new Error(`pdf_export_failed_${response.status}`);
      const blob = await response.blob();
      const url = URL.createObjectURL(blob);
      const anchor = document.createElement('a');
      anchor.href = url;
      anchor.download = `elfie-lab-summary-${artifact.job_id}.pdf`;
